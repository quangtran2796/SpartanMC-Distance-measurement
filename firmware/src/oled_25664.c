...
{
	//D/C# bit = 0;
	spi_peri_write(&SPI_MASTER, Data);
}
...
{	
	//D/C# bit = 1
	spi_peri_write(&SPI_MASTER, 0x100 + Data);
}
...
unsigned char i,j;
	...
	for(i=0;i<64;i++)
	{
		for(j=0;j<120;j++)
		{
			oled_Data_25664(0x05);
			oled_Data_25664(0x05);
		}
		for(j=0;j<120;j++)
		{
			oled_Data_25664(0x50);
			oled_Data_25664(0x50);
		}
	}
}
...
{
const char *Src_Pointer=0;
unsigned char i,Font,MSB_1,LSB_1,MSB_2,LSB_2;

	switch(a)
	{
		case 1:
			Src_Pointer=Ascii_1[(b-32)];		//modified
			break;
		case 2:
...
{
        /*
         * Reset für 100µs auslösen, 300ms warten
         * Abschnitt 8.9 (Seite 30) Datenblatt
